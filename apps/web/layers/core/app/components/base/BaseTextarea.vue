...
const props = withDefaults(
  defineProps<{ label?: string; hint?: string; error?: string; placeholder?: string; rows?: number; maxlength?: number; disabled?: boolean }>(),
  { rows: 4 },
);
...
<template>
  <div class="w-full">
    <label v-if="label" :for="id" class="mb-1.5 block text-sm font-medium text-ink-soft">{{ label }}</label>
    <textarea
      :id="id"
      v-model="model"
      :rows="rows"
      ...
      class="w-full resize-y rounded-lg border bg-surface px-3.5 py-2.5 text-sm text-ink
