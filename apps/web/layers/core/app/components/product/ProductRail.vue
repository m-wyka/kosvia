...
function scrollBy(direction: -1 | 1) {
  track.value?.scrollBy({ left: direction * Math.round(track.value.clientWidth * 0.8), behavior: 'smooth' });
}

onMounted(updateEdges);
watch(() => props.products, () => nextTick(updateEdges));
</script>
  ...
  <section class="min-w-0">
    ...
    >
      <template v-if="loading">
        <div v-for="index in 5" :key="`s-${index}`" class="w-[13.5rem] shrink-0 sm:w-60">
          <ProductCardSkeleton />
        </div>
      </template>
      <ProductCard
