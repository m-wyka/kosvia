...
/** A persistent tray so a half-built comparison is never lost between pages. */
const compare = useCompareStore();
const route = useRoute();

const visible = computed(() => compare.count > 0 && !route.path.startsWith('/compare'));
...
<template>
  <ClientOnly>
    ...
    >
      <div
        v-if="visible"
        class="fixed inset-x-0 bottom-16 z-30 px-4 pb-3 lg:bottom-0 lg:pb-6"
      >
        <div
