...
1. **Pierwsze prawdziwe produkty — w toku (2026-08-30).** `import:obf` (kremy do twarzy, bez filtra kraju): 137 produktów, 107 widocznych (78%), skład rozpoznany w 92% wierszy; 215 tokenów w `/admin/inci-queue` — głównie etykiety bez przecinków („niacinamide trehalose panthenol”) i literówki, czyli robota dla parsera/aliasów, nie słownika. Opisy EN+PL: 139 z seeda (`prisma/seed/data/ingredients.pl.json`) + 130 najczęstszych z etykiet (`prisma/seed/data/ingredient-prose/obf-batch-1.json`); oba pliki wgrywa `ingredients:describe --from=…` i **po `db:reset` trzeba je wgrać ponownie po `import:cosing`**. Dalej: kolejne partie wg `SELECT` z liczbą wystąpień (pisane w sesji Claude Code, bez klucza API), więcej kategorii OBF. Do scalenia: wpisy z seeda o nazwach potocznych („Shea Butter”, „Jojoba Seed Oil”, „Green Tea Leaf Extract”, „Mineral Oil”, „Coenzyme Q10”, „Coconut Oil”, „Argan Oil”, „Licorice Root Extract”, „Oat Kernel Extract”, „Sunflower Seed Oil”) dublują kanoniczne INCI z CosIng — docelowo alias + przepięcie `product_ingredients` seeda.
