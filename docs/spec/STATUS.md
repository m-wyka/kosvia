...
2. **`ProductVariant`** (`03` §3) — kroki 1–2 z 3 **zrobione** (2026-08-30): tabela `product_variants` (partial unique index na `isDefault`), `variantId` na ofertach i historii cen, `npm run variants:backfill` (idempotentny), kod czyta i pisze wyłącznie warianty (`PRODUCT_INCLUDE.variants.offers`, DTO `variants[]` + pola wariantu domyślnego na `ProductSummaryDto`), OBF dopina nowy EAN jako kolejny wariant tego samego produktu, admin edytuje wariant domyślny, oferta wskazuje wariant. **Krok 3 (osobny commit, po potwierdzeniu na dev):** migracja usuwająca `products.ean/volume/volumeUnit/imageUrl`, `productId` z `product_offers`/`price_history` (+ `@@unique([productId, storeId])`), `variantId` NOT NULL; potem sprzątanie `toOfferDto` (`variantId ?? productId`) i fixture'ów. Nie ma jeszcze akcji „scal jako wariant" w adminie.
